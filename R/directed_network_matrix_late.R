#' Generate Directed Network, Late Season
#'
#' Create a directed network to show flow of vessels between fisheries in 
#' the 2015 and 2016 late seasons. Original function name: 
#' vessel_adjacency_matrix_dcrb_centered. For script 09b
#'
...
#' @examples
#' A_info <- gen_adj_matrix_late(fishtix_lg,p=p,self=TRUE,write=TRUE,size="large",noncrab_fishtix=noncrab_tix)
#' @export
gen_adj_matrix_late <- function(tickets,p,self=FALSE,write=FALSE,outdir=NULL,size=NA,noncrab_fishtix=NULL){
  #### Subset fish tickets ####
  sub_fishtix <- filter(tickets, pcgroup == p)
  ...
  op_fishtix <- filter(tickets, pcgroup != p)
  ...
  if(!is.null(noncrab_fishtix)){  ## UPDATE JAN 07,2020 - with identify "other port" beyond those in fish tickets provided with metiers.
    noncrab_fishtix <- noncrab_fishtix %>%
      filter(!(pcgroup %in% unique(tickets$pcgroup))) %>%
  ...
  if(dropped_count > 0){
    ...
    dim(emptydf)
    emptydf$drvid <- as.character(emptydf$drvid)
  ...
  if(write){
    write.csv(vesselmat_diff, here::here(outdir,paste0(p,"_late_Amatrix_",size,".csv")))
  }
