...
#' for each metier (community) identified by the infomap algorithm. For script 01, section 7.2
#'
...
#' @export
metier_key <- function(revenue, vertices, name = TRUE){
  if(name == TRUE){
    ...
    for(i in unique(comm_names_df$community)){
      ...
      tmp_names_list <- as.character(tmp_names_df$name)
    ...
    trip_community_key <- dplyr::select(trip_community_key, node, drvid_year, community, comm_id)
    colnames(trip_community_key) <- c("trip_id", "drvid_year", "comm_num", "comm_id")
  }
  else{
