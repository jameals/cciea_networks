...
for(y in years){
  for(p in myports){
    ...
    dat <- fishtix %>%
      ...
      filter(crab_year == y & IOPAC == p) #%>% # this is the line we want to active in the for loop
    ...
    tmp_g <- participation_network_crabyear(dat, filter = TRUE, filter_subgraph =FALSE, min_vessels = 3, min_contribution = 0.10)
    if(any(is.na(tmp_g))){
      ...
      V(tmp_g)$common_name <- as.character(filtered_metier_info$species_group_2_label)
      ## save vertex colors as attribute
      vertex_cols <- vertex_color(g = tmp_g)
      V(tmp_g)$colors <- vertex_cols
      ## save objects
      rds_name <- ifelse(rm_crab,  paste0("igraph_", p, "_", y, "_rmCrab.rds"),  paste0("igraph_", p, "_", y, ".rds"))
    ...
    g_stats <- g_stats %>%
      ...
      mutate(pcgroup = rep(p, times=dim(g_stats)[1]))
