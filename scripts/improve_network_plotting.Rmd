...
Try it out on the first two graphs
```{r}
g <- new_glist[[1]]
# get port group / year info from graph object
port_group <- unique(V(g)$p)
...
# plot & save to file
png(here::here('data','networks','participation_vessel_ports','plots','comparable-labels', paste0("TEST_",port_group,"_", y,"_circular.png")),bg="transparent")  # if this resolution isn't good enough, add: width = 2000, height = 1500,res=300
if(vcount(g) == 1 | ecount(g) == 0){
  plot(g, vertex.size = V(g)$importance/(max(V(g)$importance)*0.02), 
       layout = l, #where to put the nodes on the plot
       # edge.width = sqrt(E(g)$weight)/(0.037*max(sqrt(E(g)$weight))),
