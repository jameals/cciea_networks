...
or_rev_fig <- ggplot(or_rev_df, aes(x=pcgroup, y=value/10^3, fill=pcgroup)) + 
  geom_boxplot(outlier.shape = NA) +
  geom_point(size = 2, alpha = 0.3,  position = position_jitter(seed = 1, width = .2)) +
  #facet_grid(rows=vars(metric), scales="free_y") +
  scale_fill_manual(values=pal_or) +
  ylim(c(0,max_rev/10^3)) +
  labs(title='Revenue from Non-DTS Groundfish\nFisheries: 2017-2021',
       subtitle = 'Oregon',
       x = "Port Group",
       y = "Revenue (1000s of $)",
       fill = 'Port Group'
...
nca_rev_fig <- ggplot(nca_rev_df, aes(x=pcgroup, y=value/10^3, fill=pcgroup)) + 
  geom_boxplot(outlier.shape = NA) +
  geom_point(size = 2, alpha = 0.3,  position = position_jitter(seed = 1, width = .2)) +
  #facet_grid(rows=vars(metric), scales="free_y") +
  scale_fill_manual(values=pal_nca) +
