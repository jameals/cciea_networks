...
salmon_only_mean20172021_noncon <- salmon_only_noncon %>%
  ...
  group_by(IOPAC) %>%
  summarise(
    sd_salmon_rev = sd(total_rev, na.rm=TRUE),# come back to this to include afi_revenue instead
    salmon_rev = mean(total_rev, na.rm=TRUE),# come back to this to include afi_revenue instead
...
all_pal <- pnw_palette('Bay', length(allports)- length(ca_focal))
```
...
p_vessels <- ggplot(sv_bulk_noncon_ca, 
       aes(x=crab_year, y=num_vessels, colour=IOPAC, group=IOPAC)
       ) + #,shape=factor(IOPAC, levels = myports)
  geom_pointrange(
    aes(ymin = num_vessels - 2*sd_num_vessels, ymax= num_vessels + 2*sd_num_vessels, size = salmon_rev_sizeforplotting), 
    na.rm = TRUE,
    alpha=.8, 
    position = position_dodge(width = 0.5)
    ) +
  #geom_jitter() +
  scale_size_continuous(name = "Salmon Revenue\n(Millions USD)",
                        range = c(min(sv_bulk_noncon_ca$salmon_rev_sizeforplotting),max(sv_bulk_noncon_ca$salmon_rev_sizeforplotting))) +
...
p_rev <- ggplot(sv_bulk_noncon_ca, 
       aes(x=crab_year, y=total_rev/10^6, colour=IOPAC, group=IOPAC)
       ) + #,shape=factor(IOPAC, levels = myports)
  geom_pointrange(
    aes(ymin = (total_rev - 2*sd_total_rev)/10^6, ymax= (total_rev + 2*sd_total_rev)/10^6, size = salmon_rev_sizeforplotting), 
    na.rm = TRUE,
    alpha=.8, 
    position = position_dodge(width = 0.5)
    ) +
...
p_lbs <- ggplot(sv_bulk_noncon_ca, 
  ...
  geom_pointrange(
    aes(ymin = (total_pounds - 2*sd_total_pounds)/10^6, ymax= (total_pounds + 2*sd_total_pounds)/10^6, size = salmon_rev_sizeforplotting), 
    na.rm = TRUE,
    alpha=.8, 
    position = position_dodge(width = 0.5)
    ) +
...
Across California port groups, participation in commercial fisheries by salmon vessels that were active from 2017-2022 fell by as little as 6% (Santa Barbara) to almost 70% (Monterey) during the 2022-2023 fishing season. Like participation, the total revenue from commercial fishing in 2022-2023 for these salmon vessels declined as well (50-60%) in all of these port groups except Santa Barbara. In Santa Barbara, commercial fishing revenue increased >80% in 2022-2023 compared to the average from 2017-2022.
...
Points represent averages (2017-2022) or totals (2022-2023) for each port group, and error bars reflect 2 standard deviations (2017-2022). A fishing season in defined from November of year y through November of year (y+1), specifically week 46 of year y through week 45 of year (y+1), to capture the strong influence of the Dungeness crab fishery.
...
p_vessels_all <- ggplot(sv_bulk_noncon_allbut4ca, 
  ...
  geom_text_repel(
    aes(label = ifelse(crab_year == "2022", IOPAC, ""), colour = ifelse(crab_year == "2022", IOPAC, ""), fontface = 'bold'), 
    show.legend  = FALSE,
    #nudge_x = 0.4, 
    direction = "x", 
    vjust = 1,
  ...
  scale_size_continuous(name = "Salmon Revenue\n(Millions USD)",
                        range = c(min(sv_bulk_noncon_allbut4ca$salmon_rev_sizeforplotting),max(sv_bulk_noncon_allbut4ca$salmon_rev_sizeforplotting))) +
    guides(size = guide_legend(override.aes = list(shape = 1))) +
  geom_line(alpha=0.8,
    position = position_dodge(width = 0.5))+
  #scale_colour_gradientn(colours = pal, name=str_wrap("Initial Biomass",20), guide = gc)+
  xlab("Time Period") +
    scale_x_discrete(labels = c('2017-2022','2022-2023')) +
  ylab("Number of Active Salmon Vessels") +
  scale_colour_manual(values=rev(all_pal), guide = "none") +
  #guides(colour=guide_legend(title="Port Group")) +
...
p_lbs_all <- ggplot(sv_bulk_noncon_allbut4ca, 
       aes(x=crab_year, y=log10(total_pounds), colour=IOPAC, group=IOPAC)
       ) + #,shape=factor(IOPAC, levels = myports)
  geom_text_repel(
    aes(label = ifelse(crab_year == "2022", IOPAC, ""), colour = ifelse(crab_year == "2022", IOPAC, ""), fontface = 'bold'), 
    show.legend  = FALSE,
  ...
  geom_pointrange(
    aes(ymin = log10(total_pounds - 2*sd_total_pounds), ymax= log10(total_pounds + 2*sd_total_pounds), size = salmon_rev_sizeforplotting), 
    na.rm = TRUE,
    alpha=.8, 
    position = position_dodge(width = 0.5)
    ) +
...
ggsave(paste0(figdir,'/allbut4ca_sv_part_rev.png'), height=6, width=14, units="in")
