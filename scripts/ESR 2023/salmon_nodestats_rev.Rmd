...
dat <- dat %>%
  ...
  group_by(pcgroup) %>%
  mutate(
    ma_node_strength=rollapply(node_strength,width=salmon_moving_average,mean,align='right',fill=NA),
    ma_eigen=rollapply(eigen,width=salmon_moving_average,mean,align='right',fill=NA)
  )
...
dat2 <- dat2 %>%
  ...
  group_by(pcgroup) %>%
  mutate(
    ma_ed=rollapply(ed,width=salmon_moving_average,mean,align='right',fill=NA)
  )
...
dat3 <- dat3 %>%
  ...
  group_by(IOPAC) %>%
  mutate(
    ma_adjrev=rollapply(adjrev,width=salmon_moving_average,mean,align='right',fill=NA),
    ma_adjrev_scaled=rollapply(adjrev_scaled,width=salmon_moving_average,mean,align='right',fill=NA),
    ma_pounds=rollapply(pounds,width=salmon_moving_average,mean,align='right',fill=NA),
...
dat_long$metric <- recode(dat_long$metric, 
                          ...
                          ma_node_strength = paste0(salmon_moving_average,"-Year Mean Node Strength"),
                          eigen = "Importance",
                          eigen_scaled = "Scaled Importance",
                          eigen_ltmean = "Long-term Mean Importance",
                          ma_eigen = paste0(salmon_moving_average,"-Year Mean Importance")
                          )
...
dat_long2$metric <- recode(dat_long2$metric, 
                          ...
                          ma_ed = paste0(salmon_moving_average,"-Year Mean Edge Density")
                          )
...
dat_long3$metric <- recode(dat_long3$metric, 
                          ...
                          ma_adjrev = paste0(salmon_moving_average,"-Year Mean Revenue"),
                          ...
                          ma_pounds = paste0(salmon_moving_average,"-Year Mean Landed Weight"),
...
Will need to be updated if we change the value of salmon_moving_average so that it is not 5
...
## Scaled plot, moving average for economic dependence and edge density
...
```{r make_ma_fig, echo=FALSE, fig.width=10, fig.height=8}

# main network metric plot
fig_ma <- ggplot(plotdat_wide, aes(x=(`5-Year Mean Edge Density`), y=`5-Year Mean Revenue`/10^6)) + 
  geom_point() + #aes(fill = pcgroup, colour = pcgroup)
  #geom_point(aes(size = (`Long-term Mean Importance`))) +
  geom_text_repel(aes(label=pcgroup), #, size = 2*`Scaled Revenue` #, size = 2*(`Long-term Mean Importance`)
                  force = 0.5,
                  ...
                  segment.size = 0.2,
                  max.overlaps = 15) +
  geom_hline(yintercept=mean(plotdat_wide$`5-Year Mean Revenue`/10^6), alpha = 0.5) + 
  geom_vline(xintercept=mean(plotdat_wide$`5-Year Mean Edge Density`), colour='grey', alpha = 0.5) + 
  xlim(0,1) +
  labs(#subtitle = '2020-2021',
       x = "Low                                    Resilience Index                                 High\n(2016-2021)",
...
In general, we see that economic dependence on salmon from 2016-2021 (y-axis) has been high San Francisco, Bodega Bay, and Puget Sound (among other port groups), and low in Santa Barbara and Astoria (among other port groups). We note that economic dependence on salmon (i.e., salmon revenue) is positively correlated with statistics from fisheries participation networks that describe how well the salmon fishery is connected to other fisheries in the network.
