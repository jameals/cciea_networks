...
if(!require("facetscales")) {devtools::install_github("zeehio/facetscales@archived"); library(facetscales)}
...
node_metrics_scaled <- c('node_strength_scaled','node_strength_ltmean','ma_node_strength','eigen_scaled', 'eigen_ltmean', 'ma_eigen')

network_metrics <- c('ed') # ,'ed_weighted'
network_metrics_scaled <- c('ed_scaled','ed_ltmean', 'ma_ed')

landings_metrics <- c('adjrev', 'ma_adjrev','pounds',
                    'ma_pounds')
...
all_iopac <- iopac_lookup %>% dplyr::select(IOPAC) %>% distinct() %>% pull() 
...
for(i in seq(1,length(tix_filenames))){
  tmpdat3 <- read.csv(paste0(indir3, tix_filenames[i])) %>%
    ...
    summarise(
      ...
      #vessel_ids = list(unique(drvid)), #quick check to manually see if any IOPAC has <3 vessels across 5 yrs. none for whiting or salmon
      .groups = "drop"
    ) %>% 
    mutate(
      metier.cm = case_when(
        SPGRPN2 == my_spgroup[1] ~ "Salmon",
        SPGRPN2 == my_spgroup[2] ~ "Whiting",
      )
    )
    #mutate(dataset=dataset_names[i])
  if(i==1){
...
# # quick check to manually see if any IOPAC has <3 vessels across 5 yrs. none for whiting or salmon
# View(
#   dat3 %>% 
#   filter(crab_year %in% c(2016:2020)
#          ) 
...
dat <- dat %>%
  ...
  mutate(
    ...
    eigen_ltmean = attr(scale(eigen),"scaled:center") # note because we use scale() mutated varables return as matrices
  ) %>%
  arrange(year) %>%
  group_by(pcgroup, metier.cm) %>%
  mutate(
    ma_node_strength=rollapply(node_strength,width=hake_salmon_moving_average,mean,align='right',fill=NA),
    ma_eigen=rollapply(eigen,width=hake_salmon_moving_average,mean,align='right',fill=NA)
  )
...
dat2 <- dat2 %>%
  ...
  group_by(pcgroup) %>%
  mutate(
    ma_ed=rollapply(ed,width=hake_salmon_moving_average,mean,align='right',fill=NA)
  )

dat3 <- dat3 %>%
  group_by(IOPAC, metier.cm) %>%
  mutate(
  ...
  group_by(IOPAC, metier.cm) %>%
  mutate(
    ma_adjrev=rollapply(adjrev,width=hake_salmon_moving_average,mean,align='right',fill=NA),
    ma_adjrev_scaled=rollapply(adjrev_scaled,width=hake_salmon_moving_average,mean,align='right',fill=NA),
    ma_pounds=rollapply(pounds,width=hake_salmon_moving_average,mean,align='right',fill=NA),
    ma_pounds_scaled=rollapply(pounds_scaled,width=hake_salmon_moving_average,mean,align='right',fill=NA)
    )
...
dat_long$metric <- recode(dat_long$metric, 
                          ...
                          ma_node_strength = paste0(hake_salmon_moving_average,"-Year Mean Node Strength"),
                          eigen = "Importance",
                          eigen_scaled = "Scaled Importance",
                          eigen_ltmean = "Long-term Mean Importance",
                          ma_eigen = paste0(hake_salmon_moving_average,"-Year Mean Importance")
                          )
...
dat_long2$metric <- recode(dat_long2$metric, 
                          ...
                          ma_ed = paste0(hake_salmon_moving_average,"-Year Mean Edge Density")
                          )

ports_sorted <- iopac_lookup %>% 
  dplyr::select(IOPAC, PACFIN_LAT) %>% 
  group_by(IOPAC) %>% 
  summarise(lat = mean(PACFIN_LAT)) %>% 
  arrange(lat) %>% 
  dplyr::select(IOPAC) %>% 
  pull() # use arrange(-lat) if coord_flip() is NOT used
...
dat_long3$metric <- recode(dat_long3$metric, 
                          ...
                          ma_adjrev = paste0(hake_salmon_moving_average,"-Year Mean Revenue"),
                          ma_adjrev_scaled = paste0(hake_salmon_moving_average,"-Year Mean Scaled Revenue"),
                          pounds = "Landed Weight",
                          pounds_scaled = "Scaled Landed Weight",
                          pounds_ltmean = "Long-term Mean Landed Weight",
                          ma_pounds = paste0(hake_salmon_moving_average,"-Year Mean Landed Weight"),
                          ma_pounds_scaled = paste0(hake_salmon_moving_average,"-Year Mean Scaled Landed Weight")
                          )
...
Will need to be updated if we change the value of hake_salmon_moving_average so that it is not 5
...
plotdat <- dat_long %>% filter(year %in% years & pcgroup %in% all_iopac) %>% select(-dataset) %>% 
  bind_rows(dat_long3 %>% 
              dplyr::select (-SPGRPN2) %>%
              rename(
                year = crab_year,
                pcgroup = IOPAC
              ) %>%
              filter(year %in% years & pcgroup %in% all_iopac) %>%
              dplyr::select(pcgroup, year, metier.cm, metric, value)
            ) %>% 
  bind_rows(dat_long2 %>%
              filter(y %in% years & pcgroup %in% all_iopac) %>%
              mutate(metier.cm = 'Salmon') %>%
              rename(year = y) %>%
              dplyr::select(pcgroup, year, metier.cm, metric, value)
  ) %>% 
  bind_rows(dat_long2 %>%
              filter(y %in% years & pcgroup %in% all_iopac) %>%
              mutate(metier.cm = 'Whiting') %>%
              rename(year = y) %>%
              dplyr::select(pcgroup, year, metier.cm, metric, value)
  )

plotdat_wide <- plotdat %>%
  dplyr::select(-metier.cm) %>%
...
`5-Year Mean Edge Density` = replace(`5-Year Mean Edge Density`, is.na(`5-Year Mean Edge Density`) == TRUE, 0),
`Scaled Importance` = replace(`Scaled Importance`, is.na(`Scaled Importance`) == TRUE, 0),
`5-Year Mean Importance` = replace(`5-Year Mean Importance`, is.na(`5-Year Mean Importance`) == TRUE, 0)
    )


write_csv(plotdat_wide,
          here::here(data_out_dir,'plotdat_wide.csv')
          )
...
ggplot(plotdat_wide) +
  ...
  facet_wrap(vars(metier.cm )) +
  theme_classic2()

ggplot(plotdat_wide) +
  geom_point(aes(x=`5-Year Mean Node Strength`, y=`5-Year Mean Revenue`)) +
  facet_wrap(vars(metier.cm )) +
  theme_classic2()

ggplot(plotdat_wide) +
  geom_point(aes(x=`5-Year Mean Importance`, y=`5-Year Mean Revenue`)) +
  facet_wrap(vars(metier.cm )) +
  theme_classic2()
...
INTERPRETATION OF CORRELATIONS GOES HERE
...
<!-- ```{r make_scaled_fig_salmon, echo=FALSE, fig.width=10, fig.height=8} -->

<!-- # main network metric plot -->
<!-- fig_scaled_salmon <- ggplot(plotdat_wide %>% filter(metier.cm == 'Salmon'), aes(x=(`Long-term Mean Edge Density`), y=`Scaled Revenue`)) +  -->
...
## Moving average for economic dependence and edge density

### Salmon

We define a port group that is especially vulnerable to declines in the salmon fishery in 2021-2022, relative to the period 2004-2021, as one that has:
- high average salmon revenue from 2016-2021, indicating relatively high economic dependence on the salmon fishery over the last 5 years, 
- a low 5-year average resilience (or adaptive capacity) index value (where lower resilience corresponds to lower than average connectivity among fisheries based on fisheries participation networks for each port group). 
This approach is consistent with definitions of social vulnerability in the literature (e.g., Thiault et al. 2019). 


```{r make_ma_fig_salmon, echo=FALSE, fig.width=10, fig.height=8}
...
In the plot above, points in the upper left quadrant indicate IOPAC port groups that would be most vulnerable to declines in the salmon fishery following the 2020-2021 fishing season (e.g., Puget Sound, Monterey), while points in the lower right quadrant indicate IOPAC port groups that would be least vulnerable (e.g., Brookings, Santa Barbara). Port groups with higher economic dependence on salmon and higher resilience (points in the top right quadrant such as Bodega Bay) have greater potential for adaptation, while those with lower economic dependence on salmon and lower adaptive capacity  (points in the lower left quadrant such as the North WA Coast and Morro Bay) have high latent risk. 

In general, we see that economic dependence on salmon from 2016-2021 (y-axis) has been high in San Francisco, Bodega Bay, and Puget Sound (among other port groups), and low in Santa Barbara and Astoria (among other port groups). We note that economic dependence on salmon (i.e., salmon revenue) is positively correlated with statistics from fisheries participation networks that describe how well the salmon fishery is connected to other fisheries in the network.

The x-axis, a resilience index based on Edge Density, is a statistic derived from the fisheries participation network for each IOPAC port group. Edge Density is calculated as the ratio of the number of edges present to the total possible edges in the network, and represents the extent to which nodes within a network are connected by vessels that participate in multiple fisheries. Higher Edge Densities correspond to greater flexibility in fishery participation among all vessels represented in the network. Note that Edge Density scales with network size (it is easier to achieve a high density in a low complexity network), so comparisons across networks of different sizes should be interpreted carefully. With that caveat in mind, declines in Edge Density imply a simplification of network structure, which can in turn reduce resilience to environmental or regulatory shocks because of diminished flexibility to participate in other fisheries in the same port group (e.g., Fisher et al. 2021). Here we have calculated the 5-year average Edge Density from 2016-2021 for each port group. 

Grey lines on the x- and y-axes in this plot are for reference only. These lines correspond to the mean values across port groups.
...
We define a port group that is especially vulnerable to declines in the whiting fishery in 2021-2022, relative to the period 2004-2021, as one that has:
- high average whiting revenue from 2016-2021, indicating relatively high economic dependence on the whiting fishery over the last 5 years, 
- a low 5-year average resilience (or adaptive capacity) index value (where lower resilience corresponds to lower than average connectivity among fisheries based on fisheries participation networks for each port group). 
This approach is consistent with definitions of social vulnerability in the literature (e.g., Thiault et al. 2019). 


```{r make_ma_fig_whiting, echo=FALSE, fig.width=10, fig.height=8}
...
In the plot above, points in the upper left quadrant indicate IOPAC port groups that would be most vulnerable to declines in the whiting fishery following the 2020-2021 fishing season (e.g., none), while points in the lower right quadrant indicate IOPAC port groups that would be least vulnerable (e.g., Brookings, Santa Barbara). Port groups with higher economic dependence on whiting (Astoria, Newport, WA Coast) tend to have relatively high resilience have greater potential for adaptation, while those with lower economic dependence on whiting and lower adaptive capacity  (points in the lower left quadrant such as the Monterey and Morro Bay) have high latent risk. 

In general, we see that economic dependence on whiting from 2016-2021 (y-axis) has been high in Astoria, Newport, and WA Coast, and low in most others. We note that economic dependence on whiting (i.e., whiting revenue) may be positively correlated with statistics from fisheries participation networks that describe how well the whiting fishery is connected to other fisheries in the network, but it is difficult to discern this relationship with only 3 FPNs that include a whiting node.
...
```{r echo=FALSE, include=FALSE}
png(here::here(pngdir, paste0("5-Year Mean Salmon Revenue versus 5-Year Mean Network Edge Density from IOPAC Fisheries Participation Networks 2016-2021.png")),
    res=200,height=1200,width=1800)
fig_ma_salmon
dev.off()

png(here::here(pngdir, paste0("5-Year Mean Whiting Revenue versus 5-Year Mean Network Edge Density from IOPAC Fisheries Participation Networks 2016-2021.png")),
    res=200,height=1200,width=1800)
fig_ma_whiting
dev.off()
...
## Moving average for economic dependence and network importance

### Salmon

We define a port group that is especially vulnerable to declines in the salmon fishery in 2021-2022, relative to the period 2004-2021, as one that has:
- high average salmon revenue from 2016-2021, indicating relatively high economic dependence on the salmon fishery over the last 5 years, 
- a high 5-year average network importance of the salmon node (where network importance is based on eigen vector centrality of the salmon node determined from fisheries participation networks for each port group). 
This approach is consistent with definitions of social vulnerability in the literature (e.g., Thiault et al. 2019). 


```{r make_ma_fig_salmon_imp, echo=FALSE, fig.width=10, fig.height=8}
...
In the plot above, points in the upper right quadrant indicate IOPAC port groups that would be most vulnerable to declines in the salmon fishery following the 2020-2021 fishing season (e.g., Bodega Bay, San Francisco), while points in the lower left quadrant indicate IOPAC port groups that would be least vulnerable (e.g., Astoria, Santa Barbara). Port groups with higher economic dependence on salmon and lower salmon node network importance (points in the top left quadrant such as Puget Sound and WA Coast) are likely to show lower impacts across non-salmon fisheries, while those with lower economic dependence on salmon and higher salmon node network importance (points in the lower right quadrant such as the North WA Coast and Morro Bay) have high latent risk. 

In general, we see that economic dependence on salmon from 2016-2021 (y-axis) has been high in San Francisco, Bodega Bay, Monterey, and Puget Sound (among other port groups), and lower in Santa Barbara and Astoria (among other port groups). We note that economic dependence on salmon (i.e., salmon revenue) is weakly positively correlated with statistics from fisheries participation networks that describe how well the salmon fishery is connected to other fisheries in the network.

The x-axis, an index of the Importance of the salmon fishery to other fisheries in each port group based on Eigenvector Centrality, is a statistic derived from the fisheries participation network for each IOPAC port group. Higher Importance indicates the salmon fishery is well-connected to other well-connected fisheries in each port group. Note that Importance scales with network size (it is easier to achieve a high Importance in a large network), so comparisons across networks of different sizes should be interpreted carefully. With that caveat in mind, higher values of Importance for the salmon node imply a network structure where environmental or regulatory shocks can cascade from the salmon fishery to others in the same port group. Here we have calculated the 5-year average Importance value from 2016-2021 for each port group. 
...
We define a port group that is especially vulnerable to declines in the whiting fishery in 2021-2022, relative to the period 2004-2021, as one that has:
- high average whiting revenue from 2016-2021, indicating relatively high economic dependence on the whiting fishery over the last 5 years, 
- a high 5-year average network importance of the whiting node (where network importance is based on eigen vector centrality of the whiting node determined from fisheries participation networks for each port group). 
This approach is consistent with definitions of social vulnerability in the literature (e.g., Thiault et al. 2019). 


```{r make_ma_fig_whiting_imp, echo=FALSE, fig.width=10, fig.height=8}
...
In the plot above, points in the upper right quadrant indicate IOPAC port groups that would be most vulnerable to declines in the whiting fishery following the 2020-2021 fishing season (Astoria), while points in the lower left quadrant indicate IOPAC port groups that would be least vulnerable (many with trace amounts of whiting catch). Port groups with higher economic dependence on whiting and lower whiting node network importance (points in the top left quadrant, Newport and WA Coast) are likely to show lower impacts across non-whiting fisheries, while those with lower economic dependence on whiting and higher whiting node network importance have high latent risk (none in this case). 

In general, we see that economic dependence on whiting from 2016-2021 (y-axis) has been high in Astoria, Newport, and WA Coast, and lower in other port groups that presumably have some incidental catch of whiting. We note that economic dependence on whiting (i.e., whiting revenue) could be  positively correlated with statistics from fisheries participation networks such as Importance, but it is difficult to discern from this low sample size of port groups.

The x-axis, an index of the Importance of the whiting fishery to other fisheries in each port group based on Eigenvector Centrality, is a statistic derived from the fisheries participation network for each IOPAC port group. Higher Importance indicates the whiting fishery is well-connected to other well-connected fisheries in each port group. Note that Importance scales with network size (it is easier to achieve a high Importance in a large network), so comparisons across networks of different sizes should be interpreted carefully. With that caveat in mind, higher values of Importance for the whiting node imply a network structure where environmental or regulatory shocks can cascade from the whiting fishery to others in the same port group. Here we have calculated the 5-year average Importance value from 2016-2021 for each port group. 
...
```{r echo=FALSE, include=FALSE}
png(here::here(pngdir, paste0("5-Year Mean Salmon Revenue versus 5-Year Mean Salmon Node Network Importance from IOPAC Fisheries Participation Networks 2016-2021.png")),
    res=200,height=1200,width=1800)
fig_ma_salmon_imp
dev.off()

png(here::here(pngdir, paste0("5-Year Mean Whiting Revenue versus 5-Year Mean Whiting Node Network Importance from IOPAC Fisheries Participation Networks 2016-2021.png")),
    res=200,height=1200,width=1800)
fig_ma_whiting_imp
dev.off()
...
<!-- img_bodega <- magick::image_read( -->
<!--   here::here(pngdir_ports, -->
<!--              ESRimages_2020[grep("Bodega", ESRimages_2020)]) -->
<!-- ) -->

<!-- img_nwc <- magick::image_read( -->
<!--   here::here(pngdir_ports, -->
<!--              ESRimages_2020[grep("North", ESRimages_2020)]) -->
<!-- ) -->

<!-- ``` -->

<!-- Make plots and combine -->
<!-- ```{r plot_networks_brook_monterey} -->
...
<!-- Make plots and combine -->
<!-- ```{r plot_networks_bodega_nwc} -->

<!-- p_bodega_2020 <- ggplot(data.frame(x = 1:2, y = 1:2), aes(x, y)) + -->
<!--      geom_point(size = 1, colour=NA) + -->
<!--      draw_image(img_bodega , x = 1, y = 1, scale = 1.1, halign = 0.9) + -->
<!--      theme_void() -->

<!-- p_nwc_2020 <- ggplot(data.frame(x = 1:2, y = 1:2), aes(x, y)) + -->
<!--      geom_point(size = 1, colour=NA) + -->
<!--      draw_image(img_nwc , x = 1, y = 1, scale = 1.1, halign = 0.9) + -->
<!--      theme_void() -->

<!-- p_2020_bodega_nwc <- plot_grid(p_bodega_2020, p_nwc_2020, -->
<!--                             labels = c('B) Bodega Bay', 'C) North WA Coast'),  -->
<!--                             label_size = 16, label_x = c(0.1,0.1), label_y = c(1, 1), align = "hv", rel_widths = c(1, 1), ncol = 1 -->
<!-- ) #label_x = 0.4, c(0.35, 0.35, 0.35, 0.35, 0.35, 0.35) -->
...
<!-- # p_all_ma_scaled <- plot_grid(fig_ma_scaled, p_2020, -->
<!-- #                             labels = c('A) Salmon-Based Vulnerability',''),  -->
<!-- #                             label_size = 16, label_x = c(0,0.1), label_y = c(1,1), align = "hv", rel_widths = c(1.2, 1), ncol = 2 -->
<!-- # ) #label_x = 0.4, c(0.35, 0.35, 0.35, 0.35, 0.35, 0.35) -->
<!-- # p_all_ma_scaled -->

<!-- p_all_ma <- plot_grid(fig_ma, p_2020_bodega_nwc, -->
<!--                             labels = c('A) Salmon-Based Vulnerability',''),  -->
<!--                             label_size = 16, label_x = c(0,0.1), label_y = c(1,1), align = "hv", rel_widths = c(1.2, 1), ncol = 2 -->
<!-- ) #label_x = 0.4, c(0.35, 0.35, 0.35, 0.35, 0.35, 0.35) -->
<!-- p_all_ma -->
...
<!-- png(here::here(pngdir, paste0("Salmon-Based Vulnerability (5-Year Average) and 2 Example IOPAC Fisheries Participation Networks 2016-2021.png")), -->
<!--     res=300,height=2500,width=2900) -->
<!-- p_all_ma -->
<!-- dev.off() -->
