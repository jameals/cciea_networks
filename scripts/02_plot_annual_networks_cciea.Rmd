...
source(here::here('R/plot_network_bulk_comparable_v2.R')) # this script does the same as above, but has some additional functionality for re-aligning label positions.
```
<br>
...
Put the 2019, 2020, and all Astoria and all Morro Bay circular graphs in their own directories
```{r}
...
list.of.files_2020 <- list.files(here::here(pngdir_ports), "2020_circular_compare")
file.copy(here::here(pngdir_ports,list.of.files_2020), here::here(pngdir_ports, "2020 only"), overwrite = TRUE)
...
The second version of this function will change the location of the labels on the plots, so that they're easier to read. 
```{r}
plot_comparable_networks2(graphs_list, outdir='data/networks/participation_vessel_ports/plots/comparable-labels')
```
<br>
