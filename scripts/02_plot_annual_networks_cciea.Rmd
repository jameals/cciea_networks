...
for(iopac in myports){
  ...
  # y=2014 # testing script for single node networks, 1/27
    if(file.exists(here::here(indir_ports,paste0("igraph_", iopac, "_", y, "_totalrev", total_rev_cutoff,"_indivrev", indiv_rev_cutoff,"_", edgetype,"_rmCrab_", rundate, ".rds")))){
      tmpgraph <- readRDS(here::here(indir_ports,paste0("igraph_", iopac, "_", y, "_totalrev", total_rev_cutoff,"_indivrev", indiv_rev_cutoff,"_", edgetype,"_rmCrab_", rundate, ".rds")))
      V(tmpgraph)$p <- iopac
      V(tmpgraph)$year <- y
