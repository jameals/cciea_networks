...
#fig_path
thisyearsESRimages <- list.files(here::here(pngdir_ports), "2025_circular_compare")
...
box_iopac <- function(basemap, box_filter, region) {
  iopac_rect <- basemap + 
  geom_point(data = subset(box, map_version %in% region), aes(x=PRIMARY_LONGITUDE, y=PRIMARY_LATITUDE, fill = STATE), colour="black", pch=21, size = 2.5) +
  geom_text_repel(aes(x=PRIMARY_LONGITUDE, y=PRIMARY_LATITUDE, label= IOPAC), data = subset(box, map_version %in% region), size = 4.5, # AG changed text size 010425
                  box.padding = 0, point.padding = 0.3, hjust = -.25) +
...
CW_map <- ggdraw(box_iopac(base_map_iopac(c("washington", "oregon", "california"), c(-130, -115), c(NA, NA)), box_filter('CW'),c('WA', 'OR', 'NCA', 'SCA'))) + # AG changed longitude limits 010425
  draw_image(img_nwc, x = .03, y = .59, hjust = 0, vjust = 0, width = 0.37, height = 0.37) +
  draw_image(img_ast, x = .025, y = .4, hjust = 0, vjust = 0, width = 0.36, height = 0.36) +
  draw_image(img_fb, x = .03, y = .18, hjust = 0, vjust = 0, width = 0.37, height = 0.37) +
  draw_image(img_mb, x = .19, y = .35, hjust = 0, vjust = 1, width = 0.39, height = 0.39) 
