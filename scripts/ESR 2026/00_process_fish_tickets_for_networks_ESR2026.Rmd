...
myfile2 <- 'fish tickets 2003-2025 12152025.rds' # latest pull from PacFIN 12-15-2024, shared by Abigail Golden
...
crab_years <- seq(2004,2024) #seq(begin,end)

## which years of fish tickets are you processing? [numeric vector]
years <- seq(2004,2025) #seq(begin,end + 1)
...
As of December 2025, this results in dropping about 2.3M records. About 7.1M records remain.
```{r filter}
...
if(crab_year){
  ...
  for(y in crab_years){
    ...
    ## write out
    write.csv(tmp_out, paste0(out_conf_dir, paste0("fish_tickets_crab", y+1, "_processed_for_networks_",Sys.Date(),".csv")), row.names=FALSE)
  }
}
```
<br>
