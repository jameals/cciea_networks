...
# color palette
pal=pnw_palette("Starfish",length(unique(plotdat_nondtsgfish$pcgroup))) # or choose a different PNWColors palette

# main metric plot - nondtsgfish
figx_nondtsgfish <- ggplot(plotdat_nondtsgfish, aes(x=year, y=value, color=pcgroup)) + 
  geom_point(size=2) + geom_line() +
  #facet_wrap(nrow = 4, vars(pcgroup), scales="free_y") +
  facet_wrap(vars(pcgroup),labeller = label_wrap_gen(16)) +
  scale_color_manual(values=pal) +
  scale_x_continuous(breaks=seq(min(plotdat_nondtsgfish$year), max(plotdat_nondtsgfish$year), by=2), labels=seq(min(plotdat_nondtsgfish$year), max(plotdat_nondtsgfish$year), by=2)) +
  ylim(c(0,1)) +
...
# color palette
pal=pnw_palette("Starfish",length(unique(plotdat_dtsgfish$pcgroup))) # or choose a different PNWColors palette
...
figx_dtsgfish <- ggplot(plotdat_dtsgfish, aes(x=year, y=value, color=pcgroup)) + 
  ...
  facet_wrap(vars(pcgroup),labeller = label_wrap_gen(16)) +
  scale_color_manual(values=pal) +
  scale_x_continuous(breaks=seq(min(plotdat_dtsgfish$year), max(plotdat_dtsgfish$year), by=2), labels=seq(min(plotdat_dtsgfish$year), max(plotdat_dtsgfish$year), by=2)) +
  ylim(c(0,1)) +
...
figy_dtsgfish <- ggplot(plotdat_dtsgfish, aes(x=year, y=value, color=pcgroup)) + 
  ...
  scale_x_continuous(breaks=seq(min(plotdat_dtsgfish$year), max(plotdat_dtsgfish$year), by=2), labels=seq(min(plotdat_dtsgfish$year), max(plotdat_dtsgfish$year), by=2)) +
  ylim(c(0,1)) +
  labs(title='Importance of DTS Groundfish in Fisheries Participation Networks',
       subtitle = '2004-2025',
       x = "Year",
       y = metric_label
       ) +
...
```{r echo=FALSE}
png(here::here(pngdir, paste0("Edge Density in Fisheries Participation Networks 2005-2025","_",Sys.Date(),".png")),
    res=200,height=1500,width=1700)
fig_ed
dev.off()
```
...
```{r echo=FALSE}
png(here::here(pngdir, paste0("Number of Nodes in Fisheries Participation Networks 2004-2025","_",Sys.Date(),".png")),
    res=200,height=1500,width=1700)
fig_n
dev.off()
```
