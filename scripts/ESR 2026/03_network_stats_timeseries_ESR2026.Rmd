...
```{r}
filenames <- c("VesselLevelNodeStats_2004_2025_21pcgroups_totalrev5000_indivrev500_connectivity_rmCrab_10pContribution_2025-12-23.csv")
networkstats_filename <- "VesselLevelNetworkStats_2004_2025_21pcgroups_totalrev5000_indivrev500_connectivity_rmCrab_10pContribution_2025-12-23.csv"
...
write.csv(dat_esr,here::here(indir, paste0('FPN_ed_n_ns_ec_salmon_nondts_FY2026.csv')), row.names=FALSE)
...
... at WA and OR ports from 2004-2025. Node strength was derived from vessel-level networks summarized at the port level. Nodes (fisheries) are included in each network with a minimum total fisheries revenue cutoff for vessels of `$5,000` and a minimum revenue cutoff for individual fisheries of `$500`.
...
... at all ports from 2005-2025. Edge density was derived from vessel-level networks summarized at the port level. Nodes (fisheries) are included in each network with a minimum total fisheries revenue cutoff for vessels of `$5,000` and a minimum revenue cutoff for individual fisheries of `$500`.
...
```{r echo=FALSE}
png(here::here(pngdir, "Edge Density in Fisheries Participation Networks 2005-2025.png"),
...
```{r echo=FALSE}
png(here::here(pngdir, "Number of Nodes in Fisheries Participation Networks 2004-2025.png"),
    res=200,height=1500,width=1700)
fig_n
dev.off()
```
