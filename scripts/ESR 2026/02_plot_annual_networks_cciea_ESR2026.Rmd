...
Make sure to define most_recent_igraph so that the date is associated with the most recent igraph files
...
B)(ii) and C)(i) A minimum annual total fisheries revenue cutoff for each vessel of `$5,000` (USD in 2005; following Holland et al. diversification indices), and a minimum annual individual fisheries revenue cutoff for each vessel of `$500` (USD in 2005; following Holland et al. diversification indices). Edge type is connectivity.
...
most_recent_igraph <- '2026-01-15' # set date associated with igraph files
...
Put the 2025, 2024, and all Astoria and all Morro Bay circular graphs in their own directories
```{r}

list.of.files_2025 <- list.files(here::here(pngdir_ports), "2025_circular_compare")
file.copy(here::here(pngdir_ports,list.of.files_2025), here::here(pngdir_ports, "2025 only"), overwrite = TRUE)

list.of.files_2024 <- list.files(here::here(pngdir_ports), "2024_circular_compare")
file.copy(here::here(pngdir_ports,list.of.files_2024), here::here(pngdir_ports, "2024 only"), overwrite = TRUE)
...
Assemble the 2022 vs 2025 networks for 2 California ports that historically had heavy salmon participation before the last 3 years of salmon fishery closures.
```{r}

bodega_2022 <- intersect(intersect(list.files(here::here(pngdir_ports), pattern = "Bodega"), list.files(here::here(pngdir_ports), pattern = "circular")), list.files(here::here(pngdir_ports), pattern = "2022")) 
bodega_2025 <- intersect(intersect(list.files(here::here(pngdir_ports), pattern = "Bodega"), list.files(here::here(pngdir_ports), pattern = "circular")), list.files(here::here(pngdir_ports), pattern = "2025")) 

img_bb2022 <- magick::image_read(
  here::here(pngdir_ports,
             bodega_2022)
)

#Get the actual dimensions (width and height in pixels)
info_bb2022 <- magick::image_info(img_bb2022)

img_bb2025 <- magick::image_read(
  here::here(pngdir_ports,
             bodega_2025)
)

#Get the actual dimensions (width and height in pixels)
info_bb2025 <- magick::image_info(img_bb2025)

#gg_bb2022 <-image_ggplot(img_bb2022) + labs(title = "Bodega Bay 2022")
gg_bb2022 <- ggplot() +
  annotation_raster(img_bb2022, xmin = 0, xmax = info_bb2022$width, ymin = 0, ymax = info_bb2022$height) +
  coord_fixed() +
  scale_x_continuous(limits = c(0, info_bb2022$width[1]), expand = c(0,0)) +
  scale_y_continuous(limits = c(0, info_bb2022$height[1]), expand = c(0,0)) +
  theme_void() + 
  labs(title = "Bodega Bay 2022")

#gg_bb2025 <-image_ggplot(img_bb2025) + labs(title = "Bodega Bay 2025")
gg_bb2025 <- ggplot() +
  annotation_raster(img_bb2025, xmin = 0, xmax = info_bb2025$width, ymin = 0, ymax = info_bb2025$height) +
  coord_fixed() +
  scale_x_continuous(limits = c(0, info_bb2025$width[1]), expand = c(0,0)) +
  scale_y_continuous(limits = c(0, info_bb2025$height[1]), expand = c(0,0)) +
  theme_void() + 
  labs(title = "Bodega Bay 2025")

bb2022_2025 <- plot_grid(gg_bb2022, gg_bb2025)

ggsave(here::here(pngdir_ports,
                paste0('bodega bay 2022 v 2025_',Sys.Date(),'.png')), bb2022_2025, width = 6, height = 10, dpi =600)


monterey_2022 <- intersect(intersect(list.files(here::here(pngdir_ports), pattern = "Monterey"), list.files(here::here(pngdir_ports), pattern = "circular")), list.files(here::here(pngdir_ports), pattern = "2022")) 
monterey_2025 <- intersect(intersect(list.files(here::here(pngdir_ports), pattern = "Monterey"), list.files(here::here(pngdir_ports), pattern = "circular")), list.files(here::here(pngdir_ports), pattern = "2025")) 

img_mt2022 <- magick::image_read(
  here::here(pngdir_ports,
             monterey_2022)
)

#Get the actual dimensions (width and height in pixels)
info_mt2022 <- magick::image_info(img_mt2022)

img_mt2025 <- magick::image_read(
  here::here(pngdir_ports,
             monterey_2025)
)

#Get the actual dimensions (width and height in pixels)
info_mt2025 <- magick::image_info(img_mt2025)

gg_mt2022 <- ggplot() +
  annotation_raster(img_mt2022, xmin = 0, xmax = info_mt2022$width, ymin = 0, ymax = info_mt2022$height) +
  coord_fixed() +
  scale_x_continuous(limits = c(0, info_mt2022$width[1]), expand = c(0,0)) +
  scale_y_continuous(limits = c(0, info_mt2022$height[1]), expand = c(0,0)) +
  theme_void() + 
  labs(title = "Monterey 2022")

#gg_mt2025 <-image_ggplot(img_mt2025) + labs(title = "Monterey 2025")
gg_mt2025 <- ggplot() +
  annotation_raster(img_mt2025, xmin = 0, xmax = info_mt2025$width, ymin = 0, ymax = info_mt2025$height) +
  coord_fixed() +
  scale_x_continuous(limits = c(0, info_mt2025$width[1]), expand = c(0,0)) +
  scale_y_continuous(limits = c(0, info_mt2025$height[1]), expand = c(0,0)) +
  theme_void() + 
  labs(title = "Monterey 2025")

mt2022_2025 <- plot_grid(gg_mt2022, gg_mt2025)

ggsave(here::here(pngdir_ports,
                paste0('monterey 2022 v 2025_',Sys.Date(),'.png')), mt2022_2025, width = 6, height = 10, dpi =600)
