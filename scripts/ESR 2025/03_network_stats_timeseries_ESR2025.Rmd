...
write.csv(dat,here::here(indir, paste0('VesselLevelNodeStats_2004_2023_21pcgroups_totalrev5000_indivrev500_connectivity_rmCrab_10pContribution_rmdupes.csv')), row.names=FALSE)

write.csv(dat2,here::here(indir, paste0('VesselLevelNetworkStats_2004_2023_21pcgroups_totalrev5000_indivrev500_connectivity_rmCrab_10pContribution_rmdupes.csv')), row.names=FALSE)
...
dat_esr1 <- dat2 %>%
  mutate(metric = 'edge density') %>%
  dplyr:: select(Year, `Edge Density`, metric, `Port Group`) %>%
  rename(
    year = Year,
    index = `Edge Density`,
    timeseries = `Port Group` 
  )
...
write.csv(dat_esr,here::here(indir, paste0('FPN_ed_ns_ec_salmon_nondts_FY2025.csv')), row.names=FALSE)
