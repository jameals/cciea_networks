...
box_iopac <- function(basemap, box_filter, region) {
  ...
  geom_text_repel(aes(x=PRIMARY_LONGITUDE, y=PRIMARY_LATITUDE, label= IOPAC), data = subset(box, map_version %in% region), size = 4.5, 
...
CW_map <- ggdraw(box_iopac(base_map_iopac(c("washington", "oregon", "california"), c(-130, -115), c(NA, NA)), box_filter('CW'),c('WA', 'OR', 'NCA', 'SCA'))) + 
  ...
  draw_image(img_mb, x = .2, y = .35, hjust = 0, vjust = 1, width = 0.39, height = 0.39) 
